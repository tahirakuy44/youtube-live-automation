    ...
    <div className="h-full flex flex-col relative animate-in fade-in duration-500">
      <div className="flex items-center justify-between mb-6 shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-slate-100 flex items-center gap-3">
            <Activity className="text-purple-500" size={32} /> System Monitoring
          </h1>
          <p className="text-slate-400 text-sm mt-1">Real-time status of your VPS and active streams.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8 shrink-0">
        ...
        <div className="glass-panel p-5 rounded-xl border-l-4 border-l-rose-500">
          ...
          <div className="text-xl font-bold text-slate-100 truncate" title={stats.cpu}>{stats.cpu}</div>
          <p className="text-xs text-slate-500 mt-2">{stats.cpuCores} Cores Available</p>
        </div>
                ...
                schedules.map((schedule) => (
                  <tr key={schedule.id} className="hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-slate-300">{schedule.start_date}</div>
                      <div className="text-xs text-slate-500">{schedule.start_time} - {schedule.end_time}</div>
                    </td>
                    <td className="px-6 py-4">
